...
<head>
    ...
    <title>Laporan Absensi Harian dan Apel - TPP Insight CMS</title>
    ...
    <div class="flex min-h-screen">
        <aside class="hidden w-72 shrink-0 border-r border-zinc-200 bg-zinc-950 text-white lg:block">
            ...
            <nav class="space-y-1 px-3">
                ...
                <a href="{{ route('cms.laporan-absensi-harian.index') }}" class="flex items-center gap-3 rounded-md bg-white/10 px-3 py-2 text-sm font-medium">
        ...
        <main class="min-w-0 flex-1">
            <header class="border-b border-zinc-200 bg-white">
                <div class="mx-auto flex max-w-[1800px] items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
                    <div>
                        <h1 class="text-xl font-semibold tracking-tight">Laporan Absensi Harian dan Apel</h1>
                        <p class="mt-1 text-sm text-zinc-500">Data dari tombol Print laporan harian di portal Absensi.</p>
                    </div>
            ...
            <section class="mx-auto max-w-[1800px] px-4 py-6 sm:px-6 lg:px-8">
                <div class="grid gap-4 md:grid-cols-3">
                    ...
                    <div class="rounded-lg border border-zinc-200 bg-white p-4 shadow-sm">
                        <div class="flex items-center justify-between">
                            <p class="text-sm font-medium text-zinc-500">Ada Apel</p>
                ...
                <div class="mt-6 grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)] 2xl:grid-cols-[340px_minmax(0,1fr)]">
                    <form method="POST" action="{{ route('cms.laporan-absensi-harian.fetch') }}" class="rounded-lg border border-zinc-200 bg-white p-5 shadow-sm">
                        @csrf
                        <div class="flex items-center gap-3 border-b border-zinc-200 pb-4">
                            ...
                            <div>
                                <h2 class="text-base font-semibold">Ambil Laporan</h2>
                                <p class="text-sm text-zinc-500">Login SKPD lalu submit tombol Print otomatis.</p>
                            </div>
                        </div>
                        ...
                        <div class="mt-5 grid grid-cols-1 gap-3 2xl:grid-cols-2">
                            <div>
                                <label class="block text-sm font-medium text-zinc-700" for="daily_date_start">Tanggal Awal</label>
                                <input id="daily_date_start" name="date_start" type="date" value="{{ old('date_start', $dateStart) }}" required
                            ...
                            <div>
                                <label class="block text-sm font-medium text-zinc-700" for="daily_date_end">Tanggal Akhir</label>
                                <input id="daily_date_end" name="date_end" type="date" value="{{ old('date_end', $dateEnd) }}" required
                                    class="mt-2 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm outline-none focus:border-cyan-600 focus:ring-2 focus:ring-cyan-100">
                            </div>
                        </div>
                        ...
                        <div class="mt-4 grid grid-cols-2 gap-3">
                            <div>
                                <label class="block text-sm font-medium text-zinc-700" for="daily_start_skpd_id">SKPD Dari</label>
                                <input id="daily_start_skpd_id" name="start_skpd_id" type="number" min="1" value="{{ old('start_skpd_id', 24) }}" required
                                    class="mt-2 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm outline-none focus:border-cyan-600 focus:ring-2 focus:ring-cyan-100">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-zinc-700" for="daily_end_skpd_id">SKPD Sampai</label>
                                <input id="daily_end_skpd_id" name="end_skpd_id" type="number" min="1" value="{{ old('end_skpd_id', 24) }}" required
                                    class="mt-2 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm outline-none focus:border-cyan-600 focus:ring-2 focus:ring-cyan-100">
                            </div>
                        </div>
                    ...
                    <div class="min-w-0 rounded-lg border border-zinc-200 bg-white shadow-sm">
                        <div class="border-b border-zinc-200 px-5 py-4">
                            <div>
                                <h2 class="text-base font-semibold">Data Laporan</h2>
                                <p class="text-sm text-zinc-500">Menampilkan 100 data per halaman.</p>
                            </div>
                            <form method="GET" action="{{ route('cms.laporan-absensi-harian.index') }}" class="mt-4 grid gap-2 md:grid-cols-2 xl:grid-cols-6">
                                <input name="search" type="search" value="{{ request('search') }}" placeholder="Cari nama, NIP, jabatan"
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-[1320px] table-fixed divide-y divide-zinc-200 text-sm xl:min-w-full">
                                <thead class="bg-zinc-50">
                                    <tr>
                                        <th class="w-32 whitespace-nowrap px-4 py-3 text-left font-semibold text-zinc-600">Tanggal</th>
                                        <th class="w-36 whitespace-nowrap px-4 py-3 text-left font-semibold text-zinc-600">SKPD</th>
                                        <th class="w-72 whitespace-nowrap px-4 py-3 text-left font-semibold text-zinc-600">Pegawai</th>
                                        <th class="w-56 whitespace-nowrap px-4 py-3 text-left font-semibold text-zinc-600">Pangkat</th>
                                        <th class="w-80 whitespace-nowrap px-4 py-3 text-left font-semibold text-zinc-600">Jabatan</th>
                                        <th class="w-28 whitespace-nowrap px-4 py-3 text-left font-semibold text-zinc-600">Pagi</th>
                                        <th class="w-28 whitespace-nowrap px-4 py-3 text-left font-semibold text-zinc-600">Pulang</th>
                                        <th class="w-24 whitespace-nowrap px-4 py-3 text-left font-semibold text-zinc-600">Apel</th>
                                    </tr>
                                </thead>
                                    ...
                                    @forelse ($reports as $report)
                                        ...
                                        <tr class="hover:bg-cyan-50/50">
                                            <td class="whitespace-nowrap px-4 py-3 text-zinc-700">
                                                <div class="font-medium">{{ optional($report->tanggal)->format('Y-m-d') ?: '-' }}</div>
                                                <div class="text-xs text-zinc-500">{{ $report->hari ?: '' }}</div>
                                            </td>
                                            ...
                                            <td class="px-4 py-3 text-zinc-700">
                                                ...
                                                <div class="text-xs text-zinc-500">{{ $report->nip ?: '' }}</div>
                                            </td>
                                            <td class="px-4 py-3 text-zinc-700">{{ $report->pangkat ?: '-' }}</td>
                                            <td class="px-4 py-3 text-zinc-700">{{ $report->jabatan ?: '-' }}</td>
                                            <td class="whitespace-nowrap px-4 py-3 text-zinc-700">{{ $report->pagi ?: '-' }}</td>
                                            <td class="whitespace-nowrap px-4 py-3 text-zinc-700">{{ $report->pulang ?: '-' }}</td>
                                            <td class="whitespace-nowrap px-4 py-3 text-zinc-700">{{ $report->apel ?: '-' }}</td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="8" class="px-6 py-12 text-center text-sm text-zinc-500">
                                                Belum ada data laporan absensi harian.
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
